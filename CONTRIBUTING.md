...
4. Chives Network's nodes on the public testnet are running the latest version `x.y.z`.
...
7. Preferences > Settings > mypy > Targets: set to `./chives` and `./tests`
...
Chives organization members have been granted CI access to deploy `beta testnets`.
If you are not a Chives organization member, you can enquire about deploying a `beta testnet` in the public dev Keybase channel.
